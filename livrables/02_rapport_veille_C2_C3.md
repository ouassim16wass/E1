...
La veille menée pour MobilityPulse poursuit quatre objectifs, directement liés aux sous-questions de la problématique formulée dans le rapport préliminaire :
...
- **Hebdomadaire (environ 1 h)** : suivi technologique via les newsletters et les releases des outils. La collecte est préparée par un agent IA de veille, présenté en partie 2.3 ; l'heure humaine est consacrée à la validation et à l'analyse.
- **Mensuelle (environ 1 h)** : veille réglementaire sur les sites de la CNIL, de la DINUM, de l'ADEME et les textes européens.
- **Trimestrielle** : revue de synthèse diffusée aux parties prenantes selon le dispositif décrit en partie 2.5.
...
| **Actualité** | Quand a-t-elle été mise à jour ? | Non datée / plus de 3 ans | Datée de moins de 12 mois ou versionnée |
...
**Règle de rétention :** une source est retenue si elle obtient **au moins 8/12** et qu'elle a été **vérifiée en ligne à la date du 20/07/2026** (existence de l'URL, date ou version constatée). Les pistes fournies par le dossier documentaire *(pièce 09)* ont été soumises à cette même grille, conformément à la consigne qui impose de les vérifier, dater et qualifier.

---

## 2.2 Sources retenues

> **Toutes les sources ci-dessous ont été vérifiées en ligne le 20/07/2026** (existence de l'URL, autorité, date ou version constatée), puis notées avec la grille présentée en partie 2.1. Seules les sources à 8/12 ou plus sont retenues.

### Sources réglementaires et accessibilité

| Source (autorité) | Date / version constatée | Score | Apport pour MobilityPulse |
|---|---|---|---|
| CNIL, durées de conservation (cnil.fr) | Page mise à jour 02/04/2026 | 12/12 | Cycle de vie en 3 phases (base active, archivage, suppression) qui fonde le cycle de vie du cahier des charges |
...
| CNIL, anonymisation et pseudonymisation | Publiée 19/05/2020 | 11/12 | 3 critères (individualisation, corrélation, inférence) pour statuer sur les réclamations |
...
| DesignGouv, annonce du RGAA 5 (DINUM) | Article du 02/03/2026, publication prévue fin 2026 | 12/12 | Anticipation : WCAG 2.2, apps mobiles, contrôle Arcom ; déclarations 4.1.2 valables 18 mois |
...
| Doc officielle Metabase et sa licence | v0.62.5 OSS (16/07/2026), AGPL / éditions payantes | 11/12 | Alternative dashboards, self-service |
...
| Dépôt Soda Core et son fichier LICENSE | v4.17.1 (15/07/2026), ELv2 | 9/12 | Alerte licence : la v4 est sortie de l'open source strict |
...
| Observatoire des politiques locales de mobilité (Cerema/GART) | Millésime 2025 (22/07/2025) | 10/12 | Référentiel territorial des AOM pour adosser les indicateurs |

**Bilan de la qualification :** 25 sources évaluées, 25 retenues (score de 8/12 ou plus), dont 3 avec réserves documentées (Soda Core pour sa licence, Spark pour son dimensionnement, NeTEx/SIRI en compatibilité différée). Les pistes de la pièce 09 ont toutes été vérifiées : aucune n'était obsolète, mais deux évolutions majeures non mentionnées ont été détectées, le RGAA 5 annoncé et l'AI Act applicable au 02/08/2026. C'est précisément l'apport d'une veille datée.
...
| Traitement distribué | Apache Spark | 4.2.0 (14/07/2026) | Apache 2.0 | À environ 13 événements par seconde en moyenne, probablement surdimensionné ; à ne retenir que si le besoin le justifie |
| Transformations SQL | dbt-core | 1.12.0 (16/07/2026) | Apache 2.0 | Transformations versionnées, testées et documentées, qui répondent à l'irritant « faible traçabilité » |
...
| Qualité (alternative) | Soda Core | 4.17.1 (15/07/2026) | ELv2 (source-available) | La v4 a quitté l'open source strict, donc moins alignée avec la préférence du projet |
...
1. **Le lakehouse sur formats ouverts est devenu le standard** : Apache Iceberg s'impose (spec V4 présentée en avril 2026, catalogue Apache Polaris promu projet top-level en février 2026). Pour MobilityPulse : un stockage objet en Parquet/Iceberg évite le verrouillage fournisseur, les données restant requêtables par plusieurs moteurs.
2. **Montée des moteurs analytiques légers** : DuckDB (1.5.x, mars 2026) traite confortablement le million d'événements par jour sur une seule machine. Pour MobilityPulse : un cluster distribué n'est plus un prérequis à ce volume, avec un impact direct sur le budget de 85 k€ et la sobriété.
3. **Simplification opérationnelle des briques historiques** : Kafka sans ZooKeeper (KRaft), Airflow 3.x modernisé (versioning des DAGs). Dagster et Prefect restent des alternatives d'orchestration plus légères si Airflow s'avérait surdimensionné. Résultat : moins de composants à maintenir pour une équipe réduite.
4. **Vigilance sur les licences de l'outillage** : mouvement contrasté en 2026, avec dbt Fusion publié sous Apache 2.0, GX Core repris par Fivetran en restant Apache 2.0, mais Soda Core v4 passé sous ELv2. Pour MobilityPulse : vérifier la licence de chaque brique avant de la retenir (fait dans le tableau ci-dessus) ; ce critère entre dans la matrice de décision du rapport de recommandations.
5. **Généralisation des agents IA pour les tâches d'analyse documentaire** : les modèles de langage savent désormais exécuter des recherches multi-sources, vérifier des URL et produire des synthèses structurées de façon autonome. Pour MobilityPulse : opportunité d'automatiser la collecte de veille elle-même (proposition ci-dessous), en cohérence avec le calendrier de l'AI Act.
...
1. **Collecte** : des agents IA parcourent les canaux de veille retenus, à savoir les flux RSS des blogs officiels, l'API GitHub (nouvelles releases des outils suivis) et les pages CNIL, DINUM et ADEME.
2. **Vérification** : chaque information est rattachée à une URL source avec date constatée ; les affirmations sans source vérifiable sont écartées (garde-fou contre les « hallucinations »).
3. **Qualification** : application automatique de la grille de notation (autorité, actualité, fiabilité, utilité) avec un score proposé.
4. **Synthèse et diffusion** : note hebdomadaire pré-rédigée, **validée par un humain avant diffusion** sur le canal de l'équipe.
...
| Planification | **Apache Airflow** (DAG hebdomadaire) | Réutilise l'orchestrateur du socle, sans composant supplémentaire |
...
> Toutes les sources ci-dessous ont été vérifiées en ligne le 20/07/2026 (URL et dates constatées, détail dans le tableau de la partie 2.2).

### RGPD : protection des données personnelles

- **Base légale** : règlement (UE) 2016/679, avec la minimisation (art. 5), le registre des traitements (art. 30) et la sécurité (art. 32).
- **Durées de conservation** : la CNIL impose de définir un cycle de vie par traitement en trois phases, base active, archivage intermédiaire, puis suppression ou archivage définitif (page CNIL mise à jour le 02/04/2026). Pour MobilityPulse : fonde directement le tableau de cycle de vie du cahier des charges.
- **Anonymisation et pseudonymisation** : la CNIL distingue trois critères (individualisation, corrélation, inférence). Pour MobilityPulse : les réclamations (SRC06) pseudonymisées restent des données personnelles soumises au RGPD ; seule une agrégation anonymisante les en sort.
- **Sécurité** : guide CNIL de la sécurité des données personnelles, édition 2024 (25 fiches, dont API, cloud et IA). Check-list de référence pour les exigences de sécurité du cahier des charges.
...
- **Évolution repérée par la veille : le RGAA 5 est officiellement annoncé** (article DesignGouv du 02/03/2026, publication prévue fin 2026) : intégration des WCAG 2.2, extension aux applications mobiles, contrôle confié à l'Arcom. Les déclarations établies avant sa publication resteront valables 18 mois.
- Pour MobilityPulse : concevoir et auditer les tableaux de bord en RGAA 4.1.2 dès le lot 1, sans attendre le RGAA 5, et prévoir une revue de conformité à la publication du nouveau référentiel.
...
- **ADEME** : parcours « numérique responsable » dédié aux collectivités (page mise à jour 16/01/2026) : diagnostic, écoconception, achats durables ; chiffres de référence issus de l'étude ADEME-Arcep (le numérique représente 10 % de la consommation électrique française et 2,5 % de l'empreinte carbone). Source des ordres de grandeur pour l'estimation d'impact du cahier des charges.
- **Green IT, 115 bonnes pratiques d'écoconception web** (5e édition, juin 2025, en libre accès, mappée sur le RGESN) : check-list opérationnelle couvrant l'agrégation à la source, le cache et la pagination des API. Directement applicable aux dashboards et API de MobilityPulse.

### Standards mobilité et obligations de publication

- **GTFS / GTFS-RT** (MobilityData) : standard de fait pour l'échange de données transport, en évolution continue (6 révisions constatées sur 2025-2026). **NeTEx/SIRI** (CEN) restent les normes européennes exigées pour les points d'accès nationaux, d'où une architecture « bi-format » à prévoir à terme.
...
|---|---|---|---|
| Équipe data / DSI | Canal partagé : note hebdomadaire pré-rédigée par l'agent IA de veille, **validée humainement** avant diffusion | Hebdomadaire | Versions d'outils, correctifs de sécurité, évolutions GTFS |
...
**Capitalisation :** chaque source retenue est consignée dans le tableau des sources (URL, date de vérification, score). Ce tableau est revérifié à chaque revue trimestrielle : une source qui n'est plus à jour est requalifiée ou retirée.
